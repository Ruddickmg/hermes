...
describe("Hermes API Endpoints (E2E)", function()
	...
	describe("with opencode agent", function()
		...
		it("close_session endpoint callable with opencode", function()
			local hermes = setup_endpoint_test("opencode")
			...
			-- Call close_session
			ok, err = pcall(function()
				hermes.close_session("test-session-id")
			end)

			-- Single assertion at the end
			assert.is_true(ok, "close_session() should not crash: " .. tostring(err))
		end)

		it("delete_session endpoint callable with opencode", function()
			local hermes = setup_endpoint_test("opencode")
			...
			-- Call delete_session
			ok, err = pcall(function()
				hermes.delete_session("test-session-id")
			end)
		...
		it("model_configurations endpoint callable with opencode", function()
			local hermes = setup_endpoint_test("opencode")

			assert.is_nil(hermes.model_configurations("test-session-id"))
		end)

		it("configure_model endpoint callable with opencode", function()
			local hermes = setup_endpoint_test("opencode")
