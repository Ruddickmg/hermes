...
-- luacov: disable
---Fetch latest release version from GitHub asynchronously
---Uses download.download_async for non-blocking HTTP request
---@param callback function Callback function(tag: string|nil, err: string|nil)
---@private
-- luacov: enable
function M.fetch_latest_async(callback)
	local logging = require("hermes.logging")
	local fallback_version = "v0.1.0"
	...
	local job_id = download.download_async(url, temp_file, "hermes-version-check", function(success, err)
		if not success then
			-- Handle both structured error tables and plain strings
			local err_msg = err
			if type(err) == "table" and err.message then
		...
		logging.notify("Latest version: " .. tag, vim.log.levels.INFO)
		callback(tag, nil)
	end, "Fetching latest version")

	if not job_id or job_id <= 0 then
		logging.notify("Failed to start version fetch. Using fallback.", vim.log.levels.WARN)
		os.remove(temp_file)
		callback(fallback_version, nil)
	end
end
